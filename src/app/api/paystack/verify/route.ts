...
export async function POST(request: Request) {
  ...
  try {
    ...
    // If paystack returns 404 or reference not found, mark as failed? But keep pending for transient errors
    if (msg.toLowerCase().includes("not found") || msg.includes("404")) {
      // Do not auto-fail; let user retry later
      return NextResponse.json({ ok: false, error: "Transaction not found on Paystack yet." }, { status: 404 });
    }
