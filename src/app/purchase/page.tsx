...
export default async function PurchaseListingPage() {
  ...
  return (
    <div className="page-shell section-gap animate-fade-in">
      <section className="mb-8">
        <h1 className="text-4xl font-bold text-text-primary tracking-tight">Purchase Devotionals</h1>
        <p className="mt-3 text-lg text-text-muted">Select a devotional to purchase access — bundle pricing is config-driven (platform one-time fee when set).</p>
      </section>
      ...
      {error ? (
        ...
        <PurchaseTable devotionals={devotionals} settings={settings} />
      )}
    </div>
  );
}